...
export default function TuiHome() {
  ...
  const [log, setLog] = useState(() => [
    ...
    {
      kind: "dim",
      text: "type `help` and press enter, or paste a streaming URL.",
    },
  ]);
  ...
  async function runLookup(url) {
    ...
    try {
      ...
      append(
        ...
        {
          kind: "dim",
          text: `  ${Object.keys(meta.platformLinks ?? {}).length} direct link(s) resolved`,
        },
    ...
    return (
      <>
        ...
        <div className={s.songMeta}>
          {song.thumbnailUrl && (
            <img src={song.thumbnailUrl} alt="" className={s.songArt} />
          )}
          <div>
        ...
        <div className={s.platforms}>
          {Object.entries(PLATFORM_META).map(([key, p]) => {
            ...
            return (
              ...
              >
                <span
                  className={`${s.platformBullet} ${isDirect ? "" : s.search}`}
                >
                  {isDirect ? "▶" : "?"}
                </span>
                <span className={s.platformName}>{p.name}</span>
                <span
                  className={`${s.platformTag} ${isDirect ? s.direct : s.search}`}
                >
    ...
    switch (head.toLowerCase()) {
      ...
      case "?":
        ...
        HELP_LINES.forEach(([c, d]) =>
          append({ kind: "dim", text: `  ${c.padEnd(20)} ${d}` }),
        );
        break;
      case "clear":
      case "cls":
      ...
      case "paste":
        if (!arg) {
          append({ kind: "warn", text: "usage: lookup <url>" });
          break;
        }
        runLookup(arg);
        break;
      case "share":
        ...
        break;
      case "jam": {
        if (!FLAGS.JAM_SESSION) {
          append({ kind: "err", text: "✗ jam sessions are disabled" });
          break;
        }
        ...
        if (sub === "new") {
          ...
          if (!code) {
            append({ kind: "warn", text: "usage: jam join <code>" });
            break;
          }
